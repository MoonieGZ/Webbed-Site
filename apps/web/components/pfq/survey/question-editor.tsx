...
export function QuestionEditor({
  ...
  const addEditChoice = () => {
    ...
    if (editingQuestion.choices.length >= 10) {
      toast.error("Maximum 10 choices allowed", toastStyles.error)
      return
    }
    setEditingQuestion({
      ...editingQuestion,
  ...
  const addChoice = () => {
    if (newQuestion.choices.length >= 10) {
      toast.error("Maximum 10 choices allowed", toastStyles.error)
      return
    }
    setNewQuestion({
            ...
            {newQuestion.question_type === "choice" && (
              <div className="space-y-3">
                ...
                {newQuestion.choices
                  ...
                  .map((choice, index) => {
                    const sortedChoices = [...newQuestion.choices].sort(
                      (a, b) => a.order_index - b.order_index,
                    )
                    return (
                        ...
                        <div className="flex flex-col gap-1">
                          <Button
                            ...
                            disabled={
                              questions
                                .sort((a, b) => a.order_index - b.order_index)
                                .findIndex((q) => q.id === question.id) === 0
                            }
                            className="h-6 px-2"
                            title="Move up"
                          >
