...
const MATERIAL_TYPES: Array<{ key: "weekly_boss" | "collectible" | "boss_drop"; label: string }> = [
...
export function WwGroupsTable() {
  const { loading, q, setQ, groupedByElement, groupsByType, materialsByType, current, updateAssignment } = useWwAdminGroups()

  return (
    <Card>
      <CardHeader>
        <CardTitle>WW Character Groupings</CardTitle>
        <CardDescription>Assign material groups to characters. Grouped by element.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="w-full max-w-md">
          <Input placeholder="Search character" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>

        {Object.keys(groupedByElement).length === 0 ? (
          <div className="text-sm text-muted-foreground">{loading ? "Loading..." : "No characters found."}</div>
        ) : null}
          ...
          {Object.entries(groupedByElement).map(([element, chars]) => (
            <div key={element} className="space-y-2">
              <div className="flex items-center gap-2">
                <img src={getElementIconUrl(element)} alt={element} className="h-6 w-6" />
              ...
              <div className="rounded-md border overflow-hidden">
                ...
                <div>
                  {chars.map((c) => (
                    <div key={c.id} className="grid grid-cols-[180px_repeat(4,minmax(160px,1fr))] items-center gap-3 px-3 py-2 border-t">
                      <div className="flex items-center gap-2">
                        <img src={getCharacterIconUrl(element, c.name)} alt={c.name} className="h-8 w-8" />
                        ...
                        return (
                          <div key={mt.key} className="py-1">
                            <Select
                              ...
                              onValueChange={(val) => updateAssignment(c.id, mt.key, val === "none" ? null : Number(val))}
                            >
                      ...
                      {GROUP_TYPES.map((gt) => {
                        ...
                        return (
                          <div key={gt.key} className="py-1">
                            <Select
                              value={selection ? String(selection) : ""}
                              onValueChange={(val) => updateAssignment(c.id, gt.key, val === "none" ? null : Number(val))}
                            >
                              <SelectTrigger className="w-full">
                                <SelectValue placeholder="Unassigned" />
                              </SelectTrigger>
