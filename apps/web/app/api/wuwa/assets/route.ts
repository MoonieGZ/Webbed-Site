...
import { getCharacterIconUrl, getElementIconUrl, getWeaponIconUrl } from "@/lib/games/ww/icons"
...
export async function GET() {
	try {
		...
		const characterGroups: Record<number, Record<string, Array<{ groupId: number; groupName: string; materials: Array<{ id: number; name: string; rarity: number }> }>>> = {}
		for (const r of charGroupRows) {
			...
			if (!groupEntry) {
				groupEntry = { groupId: r.groupId, groupName: r.groupName, materials: [] }
		...
		)) as Array<{ id: number; name: string; weapon_type: string; rarity: number }>
		...
		const weaponGroups: Record<number, Record<string, Array<{ groupId: number; groupName: string; materials: Array<{ id: number; name: string; rarity: number }> }>>> = {}
		...
		const weaponsByType: Record<string, Array<{ id: number; name: string; type: string; rarity: number; icon: string; groups: Record<string, Array<{ groupId: number; groupName: string; materials: Array<{ id: number; name: string; rarity: number }> }>> }>> = {}
		...
		const groupsByType: Record<string, Array<{ groupId: number; groupName: string; materials: Array<{ id: number; name: string; rarity: number }> }>> = {}
		for (const r of groupRows) {
			...
			g.materials.push({ id: r.materialId, name: r.materialName, rarity: r.materialRarity })
		}
		...
		const materialsByType: Record<string, Array<{ id: number; name: string; rarity: number }>> = {}
		...
		return NextResponse.json({ characters, weaponsByType, groupsByType, materialsByType })
	} catch (err) {
