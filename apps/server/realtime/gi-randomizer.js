...
function selectRandomCharacters(characters, settings) {
  ...
  const enabled = characters.filter((c) => {
    const enabledFlag = Boolean(
      (settings.characters.enabled && settings.characters.enabled[c.name]) ??
        true,
    )
  ...
  const travelers = enabled.filter(
    (c) => typeof c.name === "string" && c.name.startsWith("Traveler ("),
  )
  const nonTravelers = enabled.filter(
    (c) => !(typeof c.name === "string" && c.name.startsWith("Traveler (")),
  )
  ...
  if (settings.rules.limitFiveStars) {
    ...
    if (
      ...
      return null
    const pick5 = shuffleArray(fiveStars).slice(
      0,
...
function selectRandomBosses(bosses, settings) {
  ...
  const enabled = bosses.filter((b) =>
    Boolean(
      (settings.bosses.enabled && settings.bosses.enabled[b.name]) ?? true,
    ),
  )
  const filtered = settings.rules.coopMode
    ? enabled.filter((b) => Boolean(b.coop))
    : enabled
