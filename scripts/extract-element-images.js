...
function parseArgs(argv) {
  ...
  if (!args.in) {
    throw new Error('Usage: node scripts/extract-element-images.js --in <input.html> [--out <output.txt>]');
  }
  return args;
}
...
function main() {
  ...
  const outPath = args.out
    ...
    : path.join(path.dirname(inputPath), `${path.basename(inputPath, path.extname(inputPath))}-images.txt`);

  fs.writeFileSync(outPath, imageUrls.join('\n') + (imageUrls.length ? '\n' : ''), 'utf8');
  process.stdout.write(`Wrote ${imageUrls.length} image URLs to ${path.relative(process.cwd(), outPath)}\n`);
}
