...
function parseArgs(argv) {
  ...
  if (!args.in || !args.out || !args.type) {
    throw new Error('Usage: node scripts/weapons-to-sql.js --in <input.html> --type <Pistol|Sword|Broadblade|Rectifier|Gauntlets> --out <output.sql>');
  }
  return args;
}
...
function buildInsert(values, weaponType, sourceRelativePath) {
  ...
  const rows = values.map(({ name, rarity }) => `  ('${sqlEscape(name)}', '${weaponType}', ${rarity})`).join(',\n');
  const footer = '\n;\nCOMMIT;\n';
  return header + '\n' + rows + footer;
}
...
function main() {
  ...
  const sql = buildInsert(values, weaponType, path.relative(process.cwd(), inputPath));
  fs.writeFileSync(outputPath, sql, 'utf8');
  process.stdout.write(`Wrote ${values.length} rows to ${path.relative(process.cwd(), outputPath)}\n`);

  // Write images file next to SQL output
  const outDir = path.dirname(outputPath);
  const imagesFile = path.join(outDir, `${weaponType.toLowerCase()}-images.txt`);
  fs.writeFileSync(imagesFile, imageUrls.join('\n') + (imageUrls.length ? '\n' : ''), 'utf8');
  process.stdout.write(`Wrote ${imageUrls.length} image URLs to ${path.relative(process.cwd(), imagesFile)}\n`);
}
