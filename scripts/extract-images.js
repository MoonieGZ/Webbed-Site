...
function parseArgs(argv) {
  ...
  if (!args.in || !args.out) {
    throw new Error(
      "Usage: node scripts/extract-images.js --in <input.html> --out <output.txt>",
    )
  }
  return args
}
...
function main() {
  ...
  process.stdout.write(
    `Wrote ${urls.length} image URLs to ${path.relative(process.cwd(), outputPath)}\n`,
  )
}
